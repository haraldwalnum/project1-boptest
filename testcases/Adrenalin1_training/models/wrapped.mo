model wrapped "Wrapped model"
	// Input overwrite
	Modelica.Blocks.Interfaces.RealInput oveTSupSetAir_u(unit="K", min=288.15, max=313.15) "AHU supply air temperature set point for heating";
	Modelica.Blocks.Interfaces.BooleanInput oveTSupSetAir_activate "Activation for AHU supply air temperature set point for heating";
	...
	Modelica.Blocks.Interfaces.RealInput dh_oveTSupSetHea_u(unit="K", min=283.15, max=333.15) "Supply temperature set point for heating";
	Modelica.Blocks.Interfaces.BooleanInput dh_oveTSupSetHea_activate "Activation for Supply temperature set point for heating";
	...
	Modelica.Blocks.Interfaces.RealOutput ahu_reaFloExtAir_y(unit="m3/s") = mod.ahu.reaFloExtAir.y "AHU extract air volume flowrate";
	...
	Modelica.Blocks.Interfaces.RealOutput ahu_reaFloSupAir_y(unit="m3/s") = mod.ahu.reaFloSupAir.y "AHU supply air volume flowrate";
	...
	Modelica.Blocks.Interfaces.RealOutput oveTSupSetAir_y(unit="K") = mod.oveTSupSetAir.y "AHU supply air temperature set point for heating";
	...
	// Original model
	Adrenalin1_training.Models.Validation.RealOccupancy mod(
		oveTSupSetAir(uExt(y=oveTSupSetAir_u),activate(y=oveTSupSetAir_activate)),
		...
		dh.oveTSupSetHea(uExt(y=dh_oveTSupSetHea_u),activate(y=dh_oveTSupSetHea_activate)),
		oveValCoi(uExt(y=oveValCoi_u),activate(y=oveValCoi_activate)),
		oveTZonSet(uExt(y=oveTZonSet_u),activate(y=oveTZonSet_activate))) "Original model with overwrites";
end wrapped;
